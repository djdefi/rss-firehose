...
#### Grounded Summary Pipeline

The local model extracts at most one concise fact from each independently labeled feed item and returns machine-readable JSON. RSS Firehose then validates item IDs, sentence structure, and numeric fidelity before rendering anything.

Individual feed summaries are assembled from those validated facts. The overall summary interleaves the same sentences across feeds rather than asking the model to synthesize unrelated stories again. This prevents names, dates, and numbers from being reassigned between articles while keeping the page varied and current.

Composite headline digests, promotional title-only items, editorial notes, incomplete excerpts, duplicate sentences, and malformed model responses are excluded automatically.
...
- **Local AI Summarization**: Optional grounded summaries generated by local llama.cpp inference
- **Caching**: Intelligent summary caching to avoid unnecessary regeneration
